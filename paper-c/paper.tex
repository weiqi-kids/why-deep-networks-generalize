...
$L^{*}_{\mathrm{FC}} = \lfloor(d-1)/2\rfloor + 1 = \lceil d/2 \rceil$
for fully-connected ReLU networks (with the per-layer exponent
$\alpha_{0} = (d+1)/(2(d-1))$ from spherical-harmonic NTK eigenvalues;
the rate carries a depth-dependent prefactor $L^{L\alpha_{0}} = \exp(\Theta(d \log d))$
for $L = \Theta(d)$, tracked explicitly in
Proposition~\ref{prop:depth-mult}) and a depth-decoupled formula
for residual networks. The
...
rank ($K$ = number of classes) at the Neural Collapse fixed point are
...
gradient-based training does not produce an NTK-regime estimator; rather, it
...
escapes the NTK regime, attaining the dimension-free rate
$n^{-2s/(2s+1)}$.
...
to escape from the NTK regime), while the former analyzes the rate
...
\item \textbf{Architecture-specific critical depth for FC and ResNet}
  ...
  $L^{*}_{\mathrm{FC}} = \lfloor(d-1)/2\rfloor + 1 = \lceil d/2 \rceil$
  for fully-connected ReLU stacks (via depth-multiplicative spectral
  decay; the depth-dependent constant $L^{L\alpha_{0}}$ is tracked
  explicitly in Proposition~\ref{prop:depth-mult}), and
...
Ba--Erdogdu--Suzuki--Wu--Zhang~\cite{ba2022mei}. The new contribution
...
in the NTK regime, the rank-one effective dimension as the mechanism
...
$C^{s}$ link function $\rho$, the standard Hermite-decay result on the
Gaussian line is $|c_{l}|^{2} = O(l^{-2s})$ in the Hermite-normalized
basis (this is the standard Sobolev source condition for the
Ornstein--Uhlenbeck operator, see e.g.\
\cite[Theorem~4.6]{AtkinsonHan2012}). In particular,
$\sum_{l} c_{l}^{2}\, l^{2s} < \infty$ for $\rho \in C^{s}(\R)$,
...
$\bw_{j} = w_{j}\bv^{*}$ for some scalar $w_{j} \in \R$, and dead
neurons have norm $O(\alpha_{\mathrm{init}})$. Write
$t := \langle\bv^{*},\bx\rangle$, $t' := \langle\bv^{*},\bx'\rangle$
for the projected coordinates, and decompose
$\bx = t\,\bv^{*} + \bx_{\perp}$, $\bx' = t'\,\bv^{*} + \bx'_{\perp}$
with $\bx_{\perp},\bx'_{\perp}$ Gaussian on the $(d-1)$-dimensional
orthogonal complement of $\bv^{*}$.

The gradient of the predictor at $\bx$ has two pieces
\begin{align*}
  \nabla_{a_{j}} f_{\theta^{*}}(\bx)
    &= p^{-1/2} \relu(w_{j} t), \\
  \nabla_{\bw_{j}} f_{\theta^{*}}(\bx)
    &= p^{-1/2} a_{j} \mathbf{1}\{w_{j} t > 0\}\,\bx,
\end{align*}
for active $j$ (both $O(\alpha_{\mathrm{init}})$ for dead $j$). Hence
the pointwise kernel decomposes as
\[
  K_{\theta^{*}}(\bx,\bx') = A(t,t') + B(t,t')\,\bx^{\top}\bx',
\]
where $A,B$ are deterministic functions of the projected coordinates
arising from summing over $j$. Decomposing
$\bx^{\top}\bx' = t\,t' + \bx_{\perp}^{\top}\bx'_{\perp}$, the population
equivalent kernel is
\[
  \bar K^{\mathrm{pop}}_{\theta^{*}}(\bx,\bx')
  = \E_{\bx_{\perp},\bx'_{\perp}}\!\bigl[K_{\theta^{*}}(\bx,\bx')\bigr]
  = A(t,t') + B(t,t')\,t\,t'
    + B(t,t')\,\E[\bx_{\perp}^{\top}\bx'_{\perp}].
\]
Since $\bx_{\perp},\bx'_{\perp}$ are independent mean-zero Gaussian,
$\E[\bx_{\perp}^{\top}\bx'_{\perp}] = 0$, and the perpendicular term
\emph{vanishes} in expectation. Hence
\[
  \bar K^{\mathrm{pop}}_{\theta^{*}}(\bx,\bx')
  = A(t,t') + B(t,t')\,t\,t'
  =: \tilde K(t,t'),
\]
which depends on $\bx,\bx'$ only through $(t,t')$. The function
$\tilde K(t,t')$ is precisely the one-dimensional ReLU NTK $\bar K(t,t')$
on the projected inputs, up to an additive constant $c_{0}$ contributed
by neurons whose $\relu$-gate is identically activated (formally,
$c_{0} = \E_{w_{j},a_{j}}[\,\cdot\,]$ aggregated across $j$, modifying
$\bar K$ only on its bias direction). The dead-neuron contribution
contributes a residual of squared norm $O(\alpha_{\mathrm{init}}^{2})$.
\end{proof}
...
\begin{itemize}
\item the bias term is $\Theta(\eta^{2s/3})$. To see this, fix the
  threshold $l_{\eta}$ at which $\bar\lambda_{l_{\eta}} = \eta$, namely
  $l_{\eta} \asymp \eta^{-1/3}$ (cf.\
  Proposition~\ref{prop:1d-df}). Splitting the bias sum at $l_{\eta}$,
  \[
    \sum_{l \ge 1} \frac{\eta^{2}\, c_{l}^{2}}{(\bar\lambda_{l} + \eta)^{2}}
    \asymp
    \sum_{l \le l_{\eta}} c_{l}^{2}
    \;+\; \eta^{2}\!\!\sum_{l > l_{\eta}} \frac{c_{l}^{2}}{\bar\lambda_{l}^{2}}.
  \]
  Under the source condition $\sum_{l} c_{l}^{2} l^{2s} < \infty$, the
  second sum is bounded by $\eta^{2}\, l_{\eta}^{6-2s}
  \asymp \eta^{2}\, \eta^{-(6-2s)/3} = \eta^{2s/3}$ (for $s < 3$,
  with the obvious adaptation for higher $s$), and the first sum is
  $O(l_{\eta}^{-2s}) = O(\eta^{2s/3})$. Hence the bias is
  $\Theta(\eta^{2s/3})$;
...
(taken to infinity). Fix smoothness $s = 1$ throughout this definition
(matching the regularity at which Theorem~\ref{thm:critical-depth}
applies). For each $L$, let $\bM_{A,L}$ denote the noise
propagation operator of the trained network on a single-index target,
with effective rank $r_{\mathrm{eff}}(\bM_{A,L})$ as in
Definition~\ref{def:reff}. The \emph{critical depth} for $A$ is
\[
  L^{*}_{A}(n, d) := \min\bigl\{ L \in \mathbb{N} :
    r_{\mathrm{eff}}(\bM_{A,L}) \leq n^{2s/(2s+1)} \big|_{s=1}
    = n^{2/3}
  \bigr\},
\]
...
$n^{-2/3}$ in the trichotomy of~\cite{paperA}.
\end{definition}
...
\begin{equation}\label{eq:lstar-fc}
  ...
  = \lceil d/2 \rceil.
\end{equation}
The implied rate carries a depth-dependent prefactor of
$L^{L\alpha_{0}} = \exp(\Theta(d \log d))$ for $L = \Theta(d)$,
tracked explicitly in Proposition~\ref{prop:depth-mult} and
Remark~\ref{rem:depth-const}.
...
\begin{assumption}[One-step rich-regime gain;
labeled \texttt{(B0)}]\label{ass:beta0}
...
\[
  ...
  \beta_{0} = \alpha_{0} + \frac{1}{d-1} = \frac{d+3}{2(d-1)},
\]
i.e.\ a $+1/(d-1)$ exponent gain over $\alpha_{0}$ in the
flattened-shell index.
\end{assumption}

\begin{remark}[Status of Assumption~\ref{ass:beta0}]\label{rem:beta0-status}
We treat this as an explicit assumption rather than a fully proved
lemma. The supporting evidence is twofold.
\emph{(i) Spiked-RMT analysis.} The one-step gradient feature analysis
of Ba--Erdogdu--Suzuki--Wu--Zhang~\cite{ba2022mei} shows that, at
information exponent $\kappa = 1$, the post-step feature kernel
exhibits a rank-one spiked deformation of the kernel-regime spectrum,
with the spike concentrated along $\bv^{*}$.
\emph{(ii) DLS spike strength.} Damian--Lee--Soltanolkotabi
\cite{DLS2022} establish that the spike contributes a multiplicative
factor of order $1/(d-1)$ to the rate exponent in the
flattened-shell index for $\kappa = 1$. Combining (i) and (ii) is
consistent with the formula $\beta_{0} = \alpha_{0} + 1/(d-1)$, but a
direct derivation in the precise singular-value-on-Jacobian framework
of~\cite{paperA} is not given in either reference; we therefore
record~(B0) as an assumption and use it as a black-box input to the
rest of this section. A first-principles derivation via the
spherical-harmonic decomposition of the post-one-step Jacobian is left
to future work.
\end{remark}
...
$L^{*}_{\mathrm{FC}}(n,d) = \lceil d/2 \rceil$, what matters is the
...
spectral decay of $\bJ_{\mathrm{total}}$. Beyond the leading
$j^{-L\alpha_{0}}$ rate, no \emph{equality}
$\sigma_{j}(\bJ_{\mathrm{total}}) = \Theta(\sigma_{j}(\bJ_{L})\cdots\sigma_{j}(\bJ_{1}))$
holds in general: such a per-index multiplicative identity would require
the right singular vectors of $\bJ_{\ell+1}$ to coincide with the left
singular vectors of $\bJ_{\ell}$ across every $\ell$, which is a
measure-zero condition. A simple $2\times 2$ example illustrates this
gap. Let
\[
  \bJ_{1} = \diag(1,\epsilon), \qquad
  \bJ_{2} = \diag(\epsilon, 1).
\]
Both factors have singular values $\{1,\epsilon\}$, so
$\sigma_{1}(\bJ_{\ell})\sigma_{2}(\bJ_{\ell}) = \epsilon$ for each
$\ell$, and the naive per-index multiplicative \emph{equality} would
predict $\sigma_{2}(\bJ_{2}\bJ_{1}) = \sigma_{2}(\bJ_{2})\sigma_{2}(\bJ_{1})
= \epsilon^{2}$. Direct computation gives
\[
  \bJ_{2}\bJ_{1} = \diag(\epsilon, \epsilon) = \epsilon\,\bI,
\]
so $\sigma_{2}(\bJ_{2}\bJ_{1}) = \epsilon$, not $\epsilon^{2}$. This is
consistent with Weyl's product \emph{inequality}, which only asserts
$\sigma_{2}(\bJ_{2}\bJ_{1}) \le \sigma_{1}(\bJ_{2})\sigma_{2}(\bJ_{1}) = 1\cdot\epsilon = \epsilon$,
and which is saturated here. The mechanism is that $\bJ_{2}$'s
\emph{top} singular direction $e_{2}$ coincides with $\bJ_{1}$'s
\emph{bottom} singular direction $e_{2}$, so the small-singular-value
direction of $\bJ_{1}$ is amplified by the top singular value of
$\bJ_{2}$. This shows the multiplicative bound on $\sigma_{j}$ can be
loose by a factor as large as $\sigma_{1}(\bJ_{\ell+1})/\sigma_{j}(\bJ_{\ell+1})$
per layer when intermediate singular vectors misalign, and motivates
our use of an upper-bound argument throughout: only the leading
$j^{-L\alpha_{0}}$ rate is universal, while sharper per-index
constants depend on the alignment of consecutive singular bases.
\end{remark}

\subsection{The $\beta_{0}$ exponent and depth scaling}

A symmetric argument applied to the cross-correlation matrix
$\bGamma_{\mathrm{total}}$ via Assumption~\ref{ass:beta0} gives the per-depth
...
\[
  ...
  L > \frac{d-1}{2}.
\]
The smallest positive integer $L$ satisfying this strict inequality is
$L = \lfloor (d-1)/2 \rfloor + 1 = \lceil d/2 \rceil$, uniformly in the
parity of $d$: when $d$ is even, $(d-1)/2$ is non-integer and
$\lceil (d-1)/2\rceil = d/2 = \lceil d/2 \rceil$ already exceeds it;
when $d$ is odd, $(d-1)/2 \in \mathbb{Z}$ so the strict inequality
forces $L \ge (d-1)/2 + 1 = (d+1)/2 = \lceil d/2 \rceil$. The
qualitative scaling is $L^{*}_{\mathrm{FC}}(n,d) = \Theta(d)$,
recovering~\eqref{eq:lstar-fc}.

\subsection{Sharpness}

The matching lower bound is provided by considering
$L < \lceil d/2 \rceil$, equivalently $L \le (d-1)/2$: in this case
\[
  \beta - \alpha \;=\; L\beta_{0} - L\alpha_{0}
  \;=\; \frac{L}{d-1} \;\le\; \frac{1}{2},
\]
so the depth-$L$ network fails the strict benign overfitting condition
$\beta > \alpha + 1/2$ of paperA's Theorem~2 (Trichotomy). By that
trichotomy the network instead lies in the tempered regime
($\beta = \alpha + 1/2$, attained when $L = (d-1)/2$ for odd $d$) or
the catastrophic regime ($\beta < \alpha + 1/2$, all other cases with
$L \le (d-1)/2$). By definition in paperA's Theorem~2~\cite[Theorem~2]{paperA}, the
benign regime is $\tr(\bM) < \infty$, equivalently
$\beta > \alpha + 1/2$ strictly. The tempered regime
($\tr(\bM) = \Theta(\log n)$, unbounded) and the catastrophic
regime ($\tr(\bM) = \infty$, divergent series) both violate this
finiteness condition, so neither sub-case lies in the benign
regime. The dimension-free feature-learning rate $n^{-2s/(2s+1)}$
of our own Theorem~\ref{thm:fl-collapse} is established under the
joint hypothesis (i) the alignment hypothesis from~\cite{paperB}
and (ii) the benign condition $\tr(\bM) < \infty$ inherited from
paperA via the unified bound of~\cite[Thm.~1]{paperA}.  When
$\beta \le \alpha + 1/2$, condition (ii) fails, hence
Theorem~\ref{thm:fl-collapse}'s benign rate is no longer guaranteed.
Hence $L < \lceil d/2 \rceil$ does not attain the benign rate,
establishing matching sharpness up to the integer threshold.
...
whose underlying use of the strict-saddle avoidance results in the
nonconvex-optimization literature requires a $C^{2}$ assumption that is
not satisfied by the ReLU activation. The appropriate replacement is
the tame-function framework of Davis--Drusvyatskiy--Kakade--Lee~\cite{DDKL2020},
...
$g_{\mathrm{attn}}(\bx) = \bigl(W_{V} X\bigr)^{\top}\,\softmax\!\bigl(\,(W_{K} X)^{\top}(W_{Q}\bx)\,/\sqrt{d}\,\bigr)$
for an input token sequence $X \in \R^{d \times T}$ (columns are
tokens) and target token $\bx$, where the softmax is taken over the
$T$-many columns of $W_{K}X$. Heuristically, the softmax $\softmax(z)$
...
eigenvalues $\to$ singular values via $\lambda \mapsto \sqrt{\lambda}$), then
the per-layer cross-correlation exponent for a transformer block becomes
\[
  \beta_{0}^{\mathrm{attn}} = \beta_{0} + \tfrac{1}{2},
\]
where $\beta_{0}$ is the FC value of Assumption~\ref{ass:beta0}. We do not
