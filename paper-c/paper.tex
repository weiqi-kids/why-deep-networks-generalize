...
$L\beta_{0} - L\alpha_{0} = L/(d-1) \le 1/2$, so by Theorem~2
of~\cite{paperA} the depth-$L$ network falls in the tempered or
catastrophic regime, with effective rank at the saturation level
$r_{\mathrm{eff}}(\bM) \asymp n^{1/(2L\alpha_{0}+1)}$ on a
power-law-decaying spectrum at the boundary $\beta = \alpha + 1/2$
(see~\cite[Proposition~3]{paperA}). For $L \le (d-1)/2$ and any
$d \ge 3$ the resulting effective-rank exponent exceeds $2/3$
(the threshold $n^{2s/(2s+1)}|_{s=1} = n^{2/3}$ in
Definition~\ref{def:critical-depth}), so the single-index target with
$s=1$ does not attain the benign threshold. Hence
$L < \lceil d/2 \rceil$ does not attain the benign rate, establishing
matching sharpness up to the integer threshold.
