...
For the homogeneous CPL target $\varphi(t) = s_+ \relu(t) + s_- \relu(-t)$, a direct computation gives $\mu_1 = (s_+ - s_-)/2$, so Assumption~\ref{ass:kappa} holds whenever $s_+ \neq s_-$, i.e., whenever $\varphi$ is not an even function.
...
The following lemma is the key technical tool of this paper.

\begin{lemma}[ReLU directional independence]\label{lem:relu-indep}
...
Theorem~\ref{thm:saddle} assumes all other active neurons are aligned
(condition (ii)).  For the general case with multiple non-aligned neurons,
the second derivative $P''(0)$ acquires cross-terms
$\sum_{k \neq j} d_k\, d^2K(u(t), \hat{w}_k)/dt^2|_0$ that can have either
sign.  We conjecture that the strict saddle property holds without
condition~(ii), but a proof would require controlling these inter-neuron
interactions.  In the overparameterized regime where gradient flow aligns
neurons sequentially (as shown in~\cite{bietti2022}), condition~(ii) is
expected to hold at each stage of training: by the time the last non-aligned
neuron reaches a near-critical configuration, earlier neurons have already
aligned.
\end{remark}
...
Consider the setting of Theorem~\ref{thm:alignment} and suppose that every
critical point of $R$ with a non-aligned active neuron satisfies the
hypotheses of Theorem~\ref{thm:saddle} (in particular, condition~(ii)).
Then subgradient descent on $R$, from Lebesgue-almost every initialization,
...
\noindent\textbf{Step 2: Saddle avoidance.}
For tame functions satisfying the K\L{} inequality, the stable manifold of
any strict saddle point has Lebesgue measure zero.  The subgradient method
with step sizes $\sum_t \eta_t = \infty$, $\sum_t \eta_t^2 < \infty$
converges to Clarke critical points from almost every
initialization~\cite{davis2020}, and the K\L{} property ensures that
convergence to strict saddles has probability zero.

\medskip

\noindent\textbf{Step 3: Landscape structure.}
By Theorem~\ref{thm:alignment}, every global minimum ($R = 0$) is aligned.
By Theorem~\ref{thm:saddle}, every non-aligned critical point satisfying
condition~(ii) is a strict saddle.  By Step~2, subgradient descent from
almost every initialization avoids these saddle points and converges to an
aligned global minimum.
\end{proof}

\begin{remark}\label{rem:convergence-caveats}
Two caveats are in order.
\emph{First}, the corollary assumes condition~(ii) of Theorem~\ref{thm:saddle}
at all non-aligned critical points.  As noted in Remark~\ref{rem:general-saddle},
this is expected in the overparameterized regime but is not proved in full
generality.
\emph{Second}, the classical saddle-avoidance result of Lee et
al.~\cite{lee2016} requires $C^2$ smoothness, which ReLU objectives do not
satisfy.  The tame-function framework of~\cite{davis2020} provides the
appropriate nonsmooth extension via the K\L{} inequality.
\end{remark}
...
\item A \textbf{convergence guarantee} (Corollary~\ref{cor:convergence}) combining the landscape results with the tame-function framework of~\cite{davis2020}.
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*)]
\item \textbf{Remove the isolation condition in the strict saddle theorem.} Theorem~\ref{thm:saddle} assumes all other active neurons are aligned (condition~(ii)). Extending the result to the general case with multiple non-aligned neurons requires bounding the cross-neuron interaction terms in the second derivative; see Remark~\ref{rem:general-saddle}.
