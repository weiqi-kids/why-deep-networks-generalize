...
\in [1, K-1]$ in general, with equality only in the symmetric
edge case where the two eigenvalue blocks coincide.
The class-averaged statement is dimension-free for
$K$-class classification at the terminal phase.
\end{abstract}
...
$\E[\bxi_{i}\bxi_{i}^{\top}] = \sigma^{2}\bI_{K}$, so that
$\bH^{1/2}\bxi_{i}$ is the effective Fisher-weighted label noise that
matches the CE link.  Let
...
Assumption~\ref{ass:nc-backbone}(b)--(c), the model output map
$f(\bx_{i};\theta) = W\,h(\bx_{i};\theta)$ has the property that
varying $W$ along the simplex-ETF directions sweeps out the full
$(K-1)$-dimensional zero-sum subspace $\mathcal{Z}_{K}$ in output
space (since the rescaled rows of $W$, namely
$(\norm{W}/\norm{\mu})\mu_{c}$ for $c \in [K]$, are exactly the $K$
vertices of the simplex ETF in $\mathcal{Z}_{K}$ and span it). Hence
the projected Jacobian $\bP_{\mathcal{Z}_{K}}\bJ(\bx_{i})$ has row
rank exactly $K-1$ at any predictor satisfying the NC backbone
hypothesis, and the same holds for
$\widetilde\bJ(\bx_{i}) = \bH(\bx_{i})^{1/2}\bJ(\bx_{i})$.
...
intersection of three prior literatures, and the contribution lies
